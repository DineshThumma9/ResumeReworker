...
const Markdown = ({
  ...
  <ReactMarkdown
    ...
    components={{
      p: ({ children }) => (
        <p
          className={`${variant === "tight" ? "mb-1 text-sm" : "mb-3 text-sm"} leading-relaxed text-foreground/80 last:mb-0`}
        >
          {children}
        </p>
      ),
      ...
      ul: ({ children }) => (
        <ul className="list-disc pl-4 mb-2 space-y-0.5 text-sm text-foreground/80">
          {children}
        </ul>
      ),
      ol: ({ children }) => (
        <ol className="list-decimal pl-4 mb-2 space-y-0.5 text-sm text-foreground/80">
          {children}
        </ol>
      ),
      li: ({ children }) => <li className="leading-relaxed">{children}</li>,
      strong: ({ children }) => (
        <span className="font-semibold text-foreground/95">{children}</span>
      ),
    }}
  >
    {children}
      ...
      <div className="w-full max-w-lg mx-auto flex flex-col gap-4 mt-10 pb-24 px-4 animate-in fade-in duration-300">
        ...
        <div className="relative border-l-2 border-border/50 pl-7 ml-3 space-y-5">
          {steps.map((step, i) => (
            <div
              key={i}
              className="relative flex items-start gap-2 animate-in fade-in duration-300"
            >
              ...
              <span
                className={`text-sm ${step.type === "error" ? "text-destructive" : step.type === "success" ? "text-foreground/70" : "text-foreground/50 animate-pulse"}`}
              >
                {step.text}
              </span>
            </div>
          ...
          <div className="rounded-2xl border border-amber-500/15 bg-amber-500/5 dark:bg-amber-500/10 shadow-xs overflow-hidden animate-in slide-in-from-bottom-3 duration-500 delay-150">
            ...
            <ul className="p-6 columns-1 md:columns-2 gap-x-10 space-y-4">
              {analysis.potential_improvements.map((pt, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-3 text-sm text-foreground/80 leading-relaxed break-inside-avoid"
                >
